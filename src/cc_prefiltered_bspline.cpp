...
CCPrefilteredBSpline::CCPrefilteredBSpline(Vector min, Vector max) : CCInterpolator(min, max) {
...
CCPrefilteredBSpline::CCPrefilteredBSpline(const CCPrefilteredBSpline &other)
	: CCInterpolator(other.min_, other.max_), coeffs_(new float[other.width_ * other.height_ * other.depth_]) {
	std::copy(other.coeffs_, other.coeffs_ + (other.width_ * other.height_ * other.depth_), coeffs_);
}
...
void CCPrefilteredBSpline::preprocess(Matrix3D &grid) {
	...
	std::copy(grid.raw_data(), grid.raw_data() + (width_ * height_ * depth_), coeffs_);

	// determine the coefficients
	samples_to_coefficients(coeffs_);
}

double CCPrefilteredBSpline::interpolate(const Vector &position, const Matrix3D &mat) const {
...
void CCPrefilteredBSpline::samples_to_coefficientsX(
...
void CCPrefilteredBSpline::samples_to_coefficientsY(
	float *volume, unsigned int pitch, unsigned int width, unsigned int height, unsigned int depth) {
	for (int z = 0; z < depth_; ++z) {
...
void CCPrefilteredBSpline::samples_to_coefficientsZ(
	float *volume, unsigned int pitch, unsigned int width, unsigned int height, unsigned int depth) {
...
void CCPrefilteredBSpline::convert_to_coefficients(float *coeffs, int data_length, int step) {
	const float lambda = (1.0f - Pole) * (1.0f - 1.0f / Pole);

	// causal initialization
	float *c = coeffs;
	float previous_c;
	*c = previous_c = lambda * initial_causal_coefficient(c, data_length, step);
...
float CCPrefilteredBSpline::initial_causal_coefficient(float *coeffs, int data_length, int step) {
...
float CCPrefilteredBSpline::initial_anticausal_coefficient(float *coeffs, int data_length, int step) {
