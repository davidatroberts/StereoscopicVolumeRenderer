...
ViewSettings Settings::read_view_settings(picojson::value::object &json_obj) {
	...
	settings.distance_to_display_mm = json_obj["distance_to_display_mm"].get<double>();
	...
	settings.world_width =
		Util::compute_world_width(settings.image_width, settings.image_height, BASE_WIDTH, BASE_HEIGHT);
	settings.image_width_mm = (settings.display_width_mm / (float)settings.display_width_px) * settings.image_width;
	settings.mm_to_world_ratio = settings.world_width / settings.image_width_mm;
	settings.IOD_world = settings.mm_to_world_ratio * settings.IOD_mm;
	settings.distance_to_display_world = settings.distance_to_display_mm * settings.mm_to_world_ratio;
...
RaySettings Settings::read_ray_settings(picojson::value::object &json_obj) {
...
VolumeSettings Settings::read_volume_settings(picojson::value::object &json_obj) {
	...
	if (interpolation_type.compare(0, lattice_type.size(), lattice_type) != 0) {
		...
		if (interpolation_type.compare("CC_trilinear") == 0) {
			...
			settings.window_radius = (int)json_obj["window_radius"].get<double>();
...
TransferSettings Settings::read_transfer_settings(picojson::value::object &json_obj) {
...
PointModifierSettings Settings::read_point_modifier_settings(picojson::value::object &json_obj) {
...
CameraModifierSettings Settings::read_camera_modifier_settings(picojson::value::object &json_obj) {
...
GraphSettings Settings::read_graph_settings(picojson::value::object &json_obj) {
	...
	for (auto mod_value : modifiers) {
		...
		if (mod_type.compare("Highlight") == 0) {
			...
			Util::assert_eq(graph_value.is<picojson::object>(), "Error in camera modifier definition");
			picojson::value::object &graph_obj = graph_value.get<picojson::object>();

			// get the settings for the graph
			settings.sphere_radius = graph_obj["sphere_radius"].get<double>();
			settings.highlight_colour = {
				graph_obj["red"].get<double>(), graph_obj["green"].get<double>(), graph_obj["blue"].get<double>(),
				2255.0};
...
ShaderSettings Settings::read_shader_settings(picojson::value::object &json_obj) {
...
void Settings::parse_node_list(GraphSettings &settings) {
	...
	bool parsed;
	std::string link_contents = Util::read_file(settings.link_file.c_str(), parsed);
	Util::assert_eq(parsed, "Failed to read LINK json contents");
	std::string no_link_contents = Util::read_file(settings.no_link_file.c_str(), parsed);
	Util::assert_eq(parsed, "Failed to read NO LINK json contents");

	// parse the json content
	picojson::array link_arr, no_link_arr;
	Util::assert_eq(Util::parse_json_array(link_contents, link_arr), "Unable to parse LINK array");
	Util::assert_eq(Util::parse_json_array(no_link_contents, no_link_arr), "Unable to parse NO LINK array");
	...
	for (unsigned int i = 0; i < link_arr.size(); ++i) {
		...
		Util::assert_eq(Util::parse_json_array(link_val, link_pair), "Unable to parse pair");
		Util::assert_eq(Util::parse_json_array(no_link_val, no_link_pair), "Unable to parse no link pair");

		// link pairs
		picojson::object pos1, pos2;
		Util::assert_eq(Util::parse_json_obj(link_pair[0], pos1), "Unable to parse pos1");
		Util::assert_eq(Util::parse_json_obj(link_pair[1], pos2), "Unable to parse pos2");

		// no link pairs
		picojson::object pos3, pos4;
		Util::assert_eq(Util::parse_json_obj(no_link_pair[0], pos3), "Unable to parse pos3");
		Util::assert_eq(Util::parse_json_obj(no_link_pair[1], pos4), "Unable to parse pos4");

		// read through link values
		Vector n1pos1(pos1["x"].get<double>(), pos1["y"].get<double>(), pos1["z"].get<double>());
		Vector n1pos2(pos2["x"].get<double>(), pos2["y"].get<double>(), pos2["z"].get<double>());
		Pair p1(Sphere(n1pos1, settings.sphere_radius), Sphere(n1pos2, settings.sphere_radius), true);

		// read through nolink values
		Vector n2pos1(pos3["x"].get<double>(), pos3["y"].get<double>(), pos3["z"].get<double>());
		Vector n2pos2(pos4["x"].get<double>(), pos4["y"].get<double>(), pos4["z"].get<double>());
		Pair p2(Sphere(n2pos1, settings.sphere_radius), Sphere(n2pos2, settings.sphere_radius), false);
...
void Settings::print(ViewSettings &s) {
	...
	std::cout << "\033[1m"
			  << "ViewSettings"
			  << "\033[0m" << std::endl;
	std::cout << std::setw(27) << std::left << "camera: " << s.camera_str << std::endl;
	std::cout << std::setw(27) << std::left << "thread" << s.thread << std::endl;

	std::cout << std::setw(27) << std::left << "image_width: " << s.image_width << std::endl;
	std::cout << std::setw(27) << std::left << "image_height: " << s.image_height << std::endl;
	std::cout << std::setw(27) << std::left << "display_width_px: " << s.display_width_px << std::endl;

	std::cout << std::setw(27) << std::left << "display_width_mm: " << s.display_width_mm << std::endl;
	std::cout << std::setw(27) << std::left << "distance_to_display_mm: " << s.distance_to_display_mm << std::endl;
	std::cout << std::setw(27) << std::left << "IOD_mm: " << s.IOD_mm << std::endl;
	std::cout << std::setw(27) << std::left << "camera_axis: " << s.camera_axis << std::endl;
	std::cout << std::setw(27) << std::left << "camera_angle: " << s.camera_angle << std::endl;
	std::cout << std::setw(27) << std::left << "background_colour: "
			  << "(" << s.background_colour.red << ", " << s.background_colour.green << ", " << s.background_colour.blue
			  << ")" << std::endl;

	std::cout << std::setw(27) << std::left << "world_width: " << s.world_width << std::endl;
	std::cout << std::setw(27) << std::left << "image_width_mm: " << s.image_width_mm << std::endl;
	std::cout << std::setw(27) << std::left << "IOD_world: " << s.IOD_world << std::endl;
	std::cout << std::setw(27) << std::left << "distance_to_display_world: " << s.distance_to_display_world
			  << std::endl;
	std::cout << std::endl;
...
void Settings::print(RaySettings &s) {
	...
	std::cout << "\033[1m"
			  << "RaySettings"
			  << "\033[0m" << std::endl;
	std::cout << std::setw(16) << std::left << "sub_sample: " << s.sub_sample << std::endl;
	std::cout << std::setw(16) << std::left << "test_ray: " << s.test_ray << std::endl;

	std::cout << std::setw(16) << std::left << "sample_step: " << s.sample_step << std::endl;
	std::cout << std::setw(16) << std::left << "subsample_rate: " << s.subsample_rate << std::endl;
	std::cout << std::endl;
...
void Settings::print(VolumeSettings &s) {
	std::cout << "\033[1m"
			  << "VolumeSettings"
			  << "\033[0m" << std::endl;
	std::cout << std::setw(22) << std::left << "volume_file: " << s.volume_file << std::endl;

	std::cout << std::setw(22) << std::left << "resolution: " << s.resolution << std::endl;
	std::cout << std::setw(22) << std::left << "ratio: " << s.ratio << std::endl;
	std::cout << std::setw(22) << std::left << "lattice: " << s.lattice_str << std::endl;
	std::cout << std::setw(22) << std::left << "interpolation: " << s.interpolation_str << std::endl;

	// print off the windowsinc settings
	if (s.interpolation == InterpolationType::CC_WINDOWSINC) {
		std::cout << std::setw(22) << std::left << "window: " << s.window_str << std::endl;
		std::cout << std::setw(22) << std::left << "window_radius: " << s.window_radius << std::endl;
	}
...
void Settings::print(GraphSettings &s) {
	std::cout << "\033[1m"
			  << "GraphSettings"
			  << "\033[0m" << std::endl;
	std::cout << std::setw(17) << std::left << "highlight_nodes: " << s.highlight_nodes << std::endl;
	std::cout << std::setw(17) << std::left << "sphere_radius: " << s.sphere_radius << std::endl;
	std::cout << std::setw(17) << std::left << "link_file: " << s.link_file << std::endl;
	std::cout << std::setw(17) << std::left << "no_link_file: " << s.no_link_file << std::endl;
	std::cout << std::endl;
}
...
void Settings::print(TransferSettings &settings) {
	...
	std::cout << std::setw(12) << std::left << "alpha_file: " << settings.alpha_file << std::endl;
	std::cout << std::endl;
}

void Settings::print(PointModifierSettings &settings) {
	std::cout << "\033[1m"
			  << "PointModifierSettings"
			  << "\033[0m" << std::endl;
	std::cout << "types: ";
...
void Settings::print(ShaderSettings &s) {
	std::cout << "\033[1m"
			  << "ShadingSettings"
			  << "\033[0m" << std::endl;
	std::cout << std::setw(16) << std::left << "illumination: " << s.illumination_str << std::endl;
	std::cout << std::setw(16) << std::left << "gradient: " << s.gradient_str << std::endl;
	std::cout << std::setw(16) << std::left << "light_position: " << s.light_position << std::endl;
	...
	std::cout << std::setw(16) << std::left << "light_colour: " << s.light_colour << std::endl;
	std::cout << std::setw(16) << std::left << "ambient_colour: " << s.ambient_colour << std::endl;
