#include "ShadingModifierDecorator.hpp"
...
volume_settings_(volume_settings) {
	...
	shader_settings_ = Settings::read_shader_settings(
		settings_obj);

	// get the illumination funciton
	illumination_ = IlluminationModelFactory::make_model(
		shader_settings_.illumination_model);
...
: basic_modifier_(other.basic_modifier_), ray_settings_(other.ray_settings_), 
...
void ShadingModifierDecorator::modify(Matrix3D &mat, Vector &ray_direction, 
	...
	// compute the shading
	Vector shade = illumination_(normal, ray_direction, light_direction,
		shader_settings_.ka, shader_settings_.kd, shader_settings_.ks,
		shader_settings_.n, shader_settings_.light_colour, 
		shader_settings_.ambient_colour);
...
void swap(ShadingModifierDecorator &first, 
