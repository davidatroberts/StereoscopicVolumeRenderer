...
Colour Caster::cast(const Vector &ray_origin, const Vector &ray_direction) {
	...
	if (bounding_box_->hit(ray_origin, ray_direction, ray_start, ray_end)) {
		...
		Vector ray_position = ray_start.map_range(
			bounding_box_->bboxmin, bounding_box_->bboxmax, bounding_box_->lattice->min_coord_,
			bounding_box_->lattice->max_coord_);
		const auto ray_end_mapped = ray_end.map_range(
			bounding_box_->bboxmin, bounding_box_->bboxmax, bounding_box_->lattice->min_coord_,
			bounding_box_->lattice->max_coord_);
		...
		Vector new_ray_dir{ray_direction.x, ray_direction.y, -ray_direction.z};
		...
		for (int i = 0; i < iterations; ++i) {
			...
			modifier_->modify(mat, new_ray_dir, ray_position, src_rgba);
			...
			ray_position += new_ray_dir * sample_step_;
		}
		...
		return {
			...
			((background_colour_.blue * (1.0 - dst_rgba.w)) + (dst_rgba.z * 255)), dst_rgba.w * 255};
	}

	return background_colour_;
}

bool Caster::cast_intersect(Vector &ray_origin, Vector &ray_direction, Pair p) {
	Vector ray_start, ray_end;
	if (bounding_box_->hit(ray_origin, ray_direction, ray_start, ray_end)) {
		Vector ray_start_mapped = ray_start.map_range(
			bounding_box_->bboxmin, bounding_box_->bboxmax, bounding_box_->lattice->min_coord_,
			bounding_box_->lattice->max_coord_);
		...
		double t1, t2;
		bool intersect_n1 = p.node1.intersect(ray_start_mapped, ray_direction, t1, t2);
		bool intersect_n2 = p.node2.intersect(ray_start_mapped, ray_direction, t1, t2);
