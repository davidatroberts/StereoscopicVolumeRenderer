...
HighlightModifierDecorator::HighlightModifierDecorator(
	CameraModifier *basic_modifier, picojson::value::object &settings_obj, ViewSettings view_settings)
...
HighlightModifierDecorator::HighlightModifierDecorator(const HighlightModifierDecorator &other)
...
void swap(HighlightModifierDecorator &first, HighlightModifierDecorator &second) {
	using std::swap;
...
HighlightModifierDecorator &HighlightModifierDecorator::operator=(HighlightModifierDecorator other) {
	swap(*this, other);
	return *this;
}

void HighlightModifierDecorator::parse_node_list(
	std::string link_file, std::string no_link_file, std::vector<Pair> &pair_list) {
	...
	std::string no_link_contents = Util::read_file(no_link_file.c_str(), parsed);
	...
	for (unsigned int i = 0; i < link_arr.size(); ++i) {
		...
		Pair p1(Sphere(n1pos1, sphere_radius_), Sphere(n1pos2, sphere_radius_), true);
		...
		Pair p2(Sphere(n2pos1, sphere_radius_), Sphere(n2pos2, sphere_radius_), false);
...
void HighlightModifierDecorator::determine_highlight_positions(double x_offset, Pair &p) {}
