...
: width_(3), height_(3), values_(new float[width_*height_]){
	std::fill(values_, values_+(width_*height_), fill);
}

Matrix3::Matrix3(std::vector<float> init_vals)
: width_(3), height_(3), values_(new float[width_*height_]) {
...
values_(new float[other.width_*other.height_]) {
...
Matrix3 Matrix3::x_matrix(float angle_rad) {
	const std::vector<float> vals = {
		1.0, 0.0, 0.0,
		0.0, (float)cos(angle_rad), (float)-sin(angle_rad),
		0.0, (float)sin(angle_rad), (float)cos(angle_rad) 
	};
...
Matrix3 Matrix3::y_matrix(float angle_rad) {
	const std::vector<float> vals = {
		(float)cos(angle_rad), 0.0, (float)sin(angle_rad),
		0.0, 1.0, 0.0,
...
Matrix3 Matrix3::z_matrix(float angle_rad) {
	const std::vector<float> vals = {
		(float)cos(angle_rad), (float)-sin(angle_rad), 0.0,
		(float)sin(angle_rad), (float)cos(angle_rad), 0.0,
		0.0, 0.0, 1.0
	};
