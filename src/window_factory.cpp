...
std::function<double(double)> WindowFactory::make_window(
	WindowType window_type, int radius, std::map<std::string, double> interpolation_settings) {
	switch (window_type) {
		case WindowType::RECTANGLE: {
			return [](double x) { return 1.0f; };
		}
		case WindowType::BARTLETT: {
			double m_factor = 1.0f / ((double)radius);

			return [m_factor](double x) { return 1.0f - (fabs(x) * m_factor); };
		}
		case WindowType::BLACKMAN: {
			...
			return [m_factor1, m_factor2](double x) {
				return (0.42 + 0.5 * std::cos(x * m_factor1) + 0.08 * std::cos(x * m_factor2));
			};
		}
		case WindowType::COSINE_BELL: {
			double m_factor = M_PI / ((double)radius);

			return [m_factor](double x) { return 1.0f + std::cos(x * m_factor); };
		}
		case WindowType::COSINE_WINDOW: {
			double m_factor = M_PI / (2.0f * (double)radius);

			return [m_factor](double x) { return std::cos(x * m_factor); };
		}
		case WindowType::HAMMING: {
			double m_factor = M_PI / (double)radius;

			return [m_factor](double x) { return 0.54f + 0.46f * std::cos(x * m_factor); };
		}
		case WindowType::HANN: {
			double m_factor = M_PI / (double)radius;

			return [m_factor](double x) { return 0.5f + 0.5f * std::cos(x * m_factor); };
		}
		...
		case WindowType::WELCH: {
			...
			return [m_factor](double x) { return 1.0f - x * m_factor * x; };
		}
