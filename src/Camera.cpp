...
Camera::Camera(Vector eye_position, Vector lookat, Vector up, Caster *caster,
	FrameBuffer *buffer, ViewSettings view_settings, RaySettings ray_settings,
	double x_offset)
...
distance_(view_settings.distance_to_display_world), caster_(caster),
buffer_(buffer), ray_settings_(ray_settings), x_offset(x_offset) {
...
void Camera::render_scene() {
	...
	for (int y=0; y<vp_height_; y++) {
		for (int x=0; x<vp_width_; x++) {
			pp.x = x_scale * ((double)x - 0.5 * ((double)vp_width_ - 1.0)) 
			+ x_offset;
	...
	for (int y=0; y<vp_height_; y++) {
		for (int x=0; x<vp_width_; x++) {
			...
			for (int p = 0; p < n; ++p) {
				for (int q = 0; q < n; ++q) {
					Vector pp;
					pp.x = x_scale * ((double)x - 0.5 * 
						((double)vp_width_ - 1.0) + ((q + 0.5) / n)) 
						+ x_offset;
					pp.y = y_scale * ((double)y - 0.5 * 
						((double)vp_height_ - 1.0) + ((p + 0.5) / n));
					Vector ray_dir = ray_direction(pp);
			...
			[this, &ray_origin, tasks, i]() {
				for (int y=(i*tasks); y<((i+1)*tasks); ++y) {
					...
					for (int x=0; x<vp_width_; ++x) {
						...
						Vector pp;
						pp.x = x_scale * ((double)x - 0.5 
							* ((double)vp_width_ - 1.0)) + x_offset;
						pp.y = y_scale * ((double)y - 0.5 
							* ((double)vp_height_ - 1.0));
						Vector ray_dir = ray_direction(pp);
...
void Camera::render_highlights(GraphSettings &graph_settings, 
	VolumeSettings &volume_settings, std::string image_side) {
	...
	for (unsigned int i=0; i<mod_pair_list.size(); ++i) {
		...
		for (unsigned int j=0; j<p.position_list.size(); ++j) {
			...
			double opacity = (((double)(old_col.red
				+old_col.green+old_col.blue))/3.0f);
		...
		buffer_copy.output_image("../images/" + image_side +"_"+i_str+"_"
			+volume_settings.interpolation_str+"_"+converter.str() +".bmp");
	}
}
...
void  Camera::determine_highlight_positions(GraphSettings &graph_settings, 
	Pair &p) {
	Vector ray_origin = eye_position_;
