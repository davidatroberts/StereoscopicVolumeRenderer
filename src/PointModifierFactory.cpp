...
PointModifier* PointModifierFactory::make_modifier(
	PointModifierSettings &settings, RaySettings ray_settings, 
	...
	for (auto mod_value: settings.parameters) {
		...
		if (mod_type.compare("Perlin") == 0) {
			mod = new PerlinModifierDecorator(mod, settings_obj, ray_settings,
				volume_settings);
		}
		else if (mod_type.compare("Shading") == 0) {
			mod = new ShadingModifierDecorator(mod, settings_obj, 
