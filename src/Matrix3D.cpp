...
Matrix3D::Matrix3D(int width, int height, int depth, float fill)
...
Matrix3D::Matrix3D(std::vector<float> init_vals, int width, int height, 
	int depth)
: width(width), height(height), depth(depth) {
	if ((width > 0) && (height > 0) && (depth > 0)) {
		values_ = new float[width*height*depth];
...
values_(new float[other.width*other.height*other.depth]) {
	std::copy(other.values_, 
		other.values_ + (other.width*other.height*other.depth), values_);
}
...
Matrix3D Matrix3D::unequal_neighbourhood(int x, int y, int z, int min_radius,
	int max_radius) {
