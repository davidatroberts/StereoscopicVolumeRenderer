...
FCCLattice::FCCLattice(Vector min_coord, Vector max_coord, Vector resolution,
                       Interpolator *interpolator)
    : Lattice(min_coord, max_coord, resolution, interpolator),
      mat_(Matrix3D(resolution_.x * 2, resolution_.y * 2, resolution_.z * 2)) {}

FCCLattice::FCCLattice(const FCCLattice &other)
...
bool FCCLattice::load(std::string file_path) {
  ...
  int data_size =
      (resolution_.x / 2) * (resolution_.y / 2) * (resolution_.z / 2);
...
double FCCLattice::intersect(const Vector &position) const {
  Vector pos =
      position.map_range(min_coord_, max_coord_, min_coord_, volume_limit_);
  return interpolator_->interpolate(pos, mat_);
}

Matrix3D &FCCLattice::raw_data() { return mat_; }
