...
FrameBuffer::FrameBuffer(int width, int height) : FrameBufferAbs(width, height), img_(new Colour[width * height]) {}

FrameBuffer::FrameBuffer(const FrameBuffer &other)
	: FrameBufferAbs(other.width, other.height), img_(new Colour[other.width * other.height]) {
...
void FrameBuffer::output_image(std::string fname) {
	...
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			const float colour[] = {
				float(img_[x * height + y].red), float(img_[x * height + y].green), float(img_[x * height + y].blue),
				255};
