...
CCWindowSinc::CCWindowSinc(Vector min, Vector max, int radius, 
	std::function<double(double)> window_func)
: CCInterpolator(min, max), radius_(radius), window_m_(radius_*2), 
...
double CCWindowSinc::interpolate(Vector &position, Matrix3D &mat) {
