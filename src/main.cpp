...
int main(int argc, char *argv[]) {
	Util::assert_eq(argc >= 3, 
		"Settings file or transfer function file missing");

	// get the required files first
	std::string input_file = std::string(argv[1]);
	std::string transfer_file = std::string(argv[2]);
	...
	bool parsed;
	std::string json_contents = Util::read_file(input_file.c_str(), parsed);
	Util::assert_eq(parsed, "Failed to read JSON file");

	// read the json settings file
	picojson::value v;
	std::string err;
	picojson::parse(v, json_contents.c_str(), json_contents.c_str() + 
		strlen(json_contents.c_str()), &err);
	...
	Util::assert_eq(volume_value.is<picojson::object>(), 
		"Volume is not an object");
	picojson::value::object &volume_obj = volume_value.get<picojson::object>();
	VolumeSettings volume_settings = Settings::read_volume_settings(
		volume_obj);
	...
	PointModifierSettings mod_settings 
		= Settings::read_point_modifier_settings(json_obj);

	// get the values for the transfer function
	std::string transfer_contents = Util::read_file(transfer_file.c_str(), 
		parsed);
	Util::assert_eq(parsed, "Failed to read transfer function JSON file");
	picojson::parse(v, transfer_contents.c_str(), transfer_contents.c_str() + 
		strlen(transfer_contents.c_str()), &err);
	Util::assert_eq(v.is<picojson::object>(), 
		"Transfer function JSON is not an object");
	picojson::value::object &transfer_json_obj = v.get<picojson::object>();
	TransferSettings transfer_settings = Settings::read_transfer_settings(
		transfer_json_obj);

	// make concrete interpolator
	InterpolatorFactory interpolator_factory(Vector(0, 0, 0), 
		volume_settings.resolution);	
	Interpolator *interpolator = interpolator_factory.make_interpolator(
		volume_settings);
	Util::assert_eq(interpolator!=NULL, "Unknown interpolator used");

	// rotate the scaling and ensure all are > 0
	Quaternion rotate_q = Quaternion::create_rotation(
		view_settings.camera_axis, view_settings.camera_angle);
	if (!(volume_settings.ratio.x == 1 && volume_settings.ratio.y == 1 && 
		volume_settings.ratio.z == 1)) {
		volume_settings.ratio = rotate_q.rotate_vector(volume_settings.ratio);
		volume_settings.ratio = Vector(fabs(volume_settings.ratio.x),
			fabs(volume_settings.ratio.y), fabs(volume_settings.ratio.z));
	}

	// make concrete lattice
	Vector min_cube_coord(0, 0, 0);
	LatticeFactory lattice_factory(min_cube_coord, volume_settings.ratio, 
		volume_settings.resolution, interpolator);
	Lattice *lattice = lattice_factory.make_lattice(volume_settings.lattice);
	Util::assert_eq(lattice!=NULL, "Unknown lattice used");
	Util::assert_eq(lattice->load(volume_settings.volume_file), 
		"Failed to load the lattice properly");

	// run any preprocessing required for the lattice and filter
	Util::assert_eq(lattice->preprocess(), 
		"Failed to preprocess the lattice");
	interpolator->preprocess(lattice->raw_data());

	// setup bounding box
	Vector bboxmin(-120*volume_settings.ratio.x, -120*volume_settings.ratio.y, 
		120*volume_settings.ratio.z);
	Vector bboxmax(120*volume_settings.ratio.x, 120*volume_settings.ratio.y, 
		-120*volume_settings.ratio.z);
	...
	Util::assert_eq(transfer_function.map_transfer_function(
		transfer_settings.red_file, 
		transfer_settings.green_file,
		transfer_settings.blue_file, transfer_settings.alpha_file), 
	...
	PointModifier *modifier = point_mod_factory.make_modifier(mod_settings, 
		ray_settings, volume_settings);

	// create the raycaster
	Caster *caster = new Caster(&bounding_box, &transfer_function,
		modifier, ray_settings.sample_step, view_settings.background_colour);
