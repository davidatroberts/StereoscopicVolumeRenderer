...
CameraModifier *CameraModifierFactory::make_modifier(CameraModifierSettings &settings, ViewSettings view_settings) {
	...
	for (auto mod_value : settings.parameters) {
		...
		if (mod_type.compare("Highlight") == 0) {
			mod = new HighlightModifierDecorator(mod, settings_obj, view_settings);
		}
	}
