...
public:
	...
	friend void swap(HighlightModifierDecorator &first, HighlightModifierDecorator &second);
	HighlightModifierDecorator &operator=(HighlightModifierDecorator other);

private:
	void parse_node_list(std::string link_file, std::string no_link_file, std::vector<Pair> &pair_list);
